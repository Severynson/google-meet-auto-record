...
### Added
- **Self-uninstall** (`LaunchAgentManager.performSelfUninstall` / `StatusWindowController`): the former "Kill bg process" button is now **"Uninstall"** and fully removes the app. Because a running process cannot delete its own bundle, it spawns a detached `/bin/bash` helper (binary lives outside the bundle) that sleeps until the app quits, then: `launchctl unload` the daemon, `pkill -x MeetRecorder` (kills both the GUI and the `--daemon` background instance — same process name), `tccutil reset Accessibility` (removes the grant), `rm -f` the LaunchAgent plist, and `rm -rf /Applications/MeetRecorder.app`. Uses `pkill -x` (exact process name) not `-f` (full command line) — the helper's own command line contains "MeetRecorder" (the rm paths), so `-f` would kill the helper mid-uninstall. The confirmation dialog now explains the uninstall (irreversible, reinstall from DMG to use again) instead of the old "kill background process" wording.

### Fixed
- **Re-trigger on window switch / after manual recording stop** (`MeetWatcher` / `MeetController`): switching away from a Meet window and back re-ran the recording automation. Root cause: `MeetWatcher.poll()` cleared a session's "tried" state whenever `findMeetSessions()` momentarily dropped it from the AX tree — which happens routinely when the Meet window is unfocused. The cleared session then looked fresh and re-recorded on refocus. Fix: replaced the flicker-sensitive `triggeredSessions`/`clearSession` scheme with an explicit per-session **armed** flag (`armedSessions`, the user-requested `new_meeting` model). A session is armed **only** when seen in a pre-call state (Meet open, no "Leave call" button = waiting room or post-call screen) and disarmed the moment the recording flow runs. Eligibility is never recomputed from live UI, so window switches, refocus, and momentary AX-tree drops no longer change it. Re-recording only happens after genuinely re-entering a pre-call state (rejoining / new meeting). Removed the now-unused session-close reset loop and `lastSeenSessionKeys`.
